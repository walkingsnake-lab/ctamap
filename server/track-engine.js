 ...
 * is empty. The result is an open path (not a closed polygon); if the ML segments
 * form a closed loop, the returned array will have the same first and last point.
 */
 ...
 * Positive = CCW in geographic (lon, lat) space (y-up, standard math orientation).
 * CTA trains travel CCW around the downtown Loop, so a positive result is correct.
 */
...
/**
 * Rotates a circuit array in-place so the point nearest to targetPt becomes index 0.
 * The circuit is assumed closed (first ≈ last); the rotated result maintains that
 * property by appending a copy of the new start at the end.
 */
function rotateCircuitToEntry(circuit, targetPt) {
  ...
  // Rotate: [bestIdx … end-1, bestIdx] so start === end (closed open-path)
  return [...circuit.slice(bestIdx), ...circuit.slice(1, bestIdx + 1)];
}

/**
 * Finds the endpoint of any own/shared segment that is closest to any ML segment
 * endpoint, within threshold. Returns the [lon, lat] pair, or null.
 */
...
function buildLineSegments(geojson) {
  ...
  for (const legend of Object.keys(C.LINE_COLORS)) {
    ...
    if (C.LOOP_LINE_CODES.includes(legend)) {
      ...
      // GR (Green Line) is bidirectional on its Lake/Wabash loop section — trains
      // run both east (to Cottage Grove) and west (to Harlem/Lake) through the same
      // track. A fixed CCW direction would be wrong for half the trains, so we keep
      // individual ML fragments and let the normal direction cascade handle GR.
      ...
      if (isUnidirectionalLoop) {
        ...
        const stitched = stitchMLCircuit(mlCoords, C.SEGMENT_CONNECT_THRESHOLD);
        if (stitched) {
          if (signedArea(stitched) < 0) stitched.reverse();
          const allApproach = coords.concat(sharedCoords);
          const entryPt = findMLEntryPoint(allApproach, mlCoords, C.SEGMENT_CONNECT_THRESHOLD);
